...
export async function GET(request: Request) {
  ...
  try {
    ...
    safePayouts.forEach((p: any) => {
      payoutsByCreator[p.creator_id] = (payoutsByCreator[p.creator_id] || 0) + Number(p.amount);
    });

    const oneWeekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
    const newSellersThisWeek = finalCreators?.filter(c => c.created_at > oneWeekAgo).length || 0;

    return NextResponse.json({
      sellerCount: sellerCount || finalCreators.length,
      ...
      newSellersThisWeek,
      creators: finalCreators?.map(c => ({
        ...c,
        revenue: revenueByCreator[c.id] || 0,
        paid_out: payoutsByCreator[c.id] || 0,
